...
const FingerPopup = () => {
    ...
    useEffect(() => {
        const checkFingerprintSupportAndPreference = async () => {
            // Check if userPin exists and useFingerprint is not set
            const userPin = await AsyncStorage.getItem('userPin');
            const useFingerprint = await AsyncStorage.getItem('useFingerprint');
            
            if (!userPin || useFingerprint !== null) {
                return; // Don't show the popup if no pin is set or fingerprint preference already exists
            }
